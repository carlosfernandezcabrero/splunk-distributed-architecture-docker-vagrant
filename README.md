...
- Splunk Enterprise
  ...
  - Configurar la IP base para el manager y el heavy forwarder. Para ello debemos irnos al fichero `splunk-enterprise/env.rb` y configurar la variable `BASE_IP` con la IP base a parti de la cual se debe conformar la IP del manager y del Heavy Forwarder. Por ejemplo si especificamos la IP base "192.168.33", al manager se le asignara la IP "192.168.33.2" y si tenemos la IP base "192.190.25", al manager se le asignara la IP "192.190.25.2".
- Universal Forwarder
  ...
  - Configurar los comandos que queremos que se hagan durante la creación de la imagen para el Universal Forwarder. Para ello debemos ir al archivo `universal-forwarder/forwarder-compose.yml` y cambiar el valor para el argumento `CONTAINER_RUN` del servicio `uf`. Si no se quiere especificar ninguno dejar el contenido que hay por defecto.
...
También podemos modificar el archivo `distsearch.conf` que se encuentra en la carpeta `splunk-enterprise/configs/sh/distsearch.conf`. Tendremos que añadir en el parámetro `servers` de la stanza `distributedSearch` las IPs de los indexadores donde queramos buscar separadas por comas sin espacios. Por defecto esta añadida la IP del indexador de desarrollo.
